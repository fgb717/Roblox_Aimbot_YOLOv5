# Roblox_Aimbot_YOLOv5
A Roblox game character detection model based on YOLOv5. Includes a model of n specifications.<br>
The ratio of the training set to the validation set is 8:2, with 1,640 images in the training set and 410 images in the validation set, totaling 2,050 images. There is only one category, 'body', which refers to the torso.  
***Because of the diversity of Roblox games and clothing, this model is difficult to fully adapt to all games, and it is inevitable that there will be some missed detections and false detections.***  
**Due to my lack of computing power, this may be the final version of the model, and later a 'small' version of the model will be trained based on the original dataset.**  

![示例图片](confusion_matrix.png)
![示例图片](labels.jpg)
![示例图片](F1_curve.png)
![示例图片](results.png)
![示例图片](1.png)
![示例图片](2.png)
![示例图片](3.png)
![示例图片](4.png)


